function [onset_ap,V__]=modonset2_onechannel(anf,f_s,DT,schwelle,kanal,width)
% schwelle~-0.04 oder 0 normal
...
%g_E=g_E_CH *20;       % scale
if visualize
    %figure,plot(t*1e3,g_E(50,:)*1e12)%auskomm
    % ylabel('current (pA)')%auskomm
win=1/sqrt(2*pi)/sigma * exp(-1/2*(range-0.5).^2/sigma^2); %ergänzt 4.8.05
...
for t=1:schritte
    ...
    G_Na = g_Na*m^3*h; %verbessert 1.7.05
    I_Na = G_Na*(V-V_Na);
    %-------------Leakage current------------
    G_lk = g_lk;
    I_lk=G_lk*(V-V_lk);
